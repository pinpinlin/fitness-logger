...
// weight: 正數＝負重；null/''＝自重；負數＝輔助（如 -20 → 自重-20，機台幫你扛 20kg）
export function formatSetToken(set) {
  const v = set.weight;
  const w = (v === null || v === undefined || v === '') ? '自重'
    : (Number(v) === 0 ? '自重'
      : (Number(v) < 0 ? `自重-${Math.abs(Number(v))}` : v));
...
// {name, doneRounds, load} → '[[名]] (型態:: HIIT) (HIIT組:: A選填) (完成:: 4輪) (負重:: 10kg)'
// load 空＝徒手，不輸出該欄位；tag 只在一場有多組 HIIT 時輸出
export function hiitLine(item, tag) {
  let line = `[[${item.name}]] (型態:: HIIT)`;
  if (tag) line += ` (HIIT組:: ${tag})`;
  line += ` (完成:: ${item.doneRounds}輪)`;
...
/* ── 本場型態 ── */
// 取有動作的 HIIT 組（session.hiits 為陣列；相容舊的單一 session.hiit）
export function hiitGroups(session) {
  const list = session.hiits || (session.hiit ? [session.hiit] : []);
  return list.filter(g => g && (g.items || []).length);
}

export function sessionTypes(session) {
  const t = [];
  if ((session.entries || []).some(e => formatSetsField(e.sets) !== '')) t.push('重訓');
  if (hiitGroups(session).length) t.push('HIIT');
...
export function buildMd(session, blocks) {
  ...
  // 單組才放場級參數（相容既有 Dataview）；多組時各組參數寫在該區塊的引言行
  if (hgs.length === 1) {
    const p = hgs[0].params;
  ...
  if (hgs.length) {
    const multi = hgs.length > 1;
    hgs.forEach((g, gi) => {
      const p = g.params;
      const tag = g.tag || String.fromCharCode(65 + gi);
      const total = fmtDuration(hiitTotalSeconds(p, g.items.length));
      L.push(multi ? `## 🔥 HIIT ${tag}` : '## 🔥 HIIT',
        `> 每項 ${p.workSec} 秒 ｜ 項間休 ${p.restSec} 秒 ｜ ${p.rounds} 輪 ｜ 輪間休 ${p.roundRestSec || 0} 秒 ｜ 總時長 ${total}`,
        ...g.items.map(it => hiitLine(it, multi ? tag : null)), '');
    });
